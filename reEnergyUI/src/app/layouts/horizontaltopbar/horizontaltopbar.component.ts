...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.currentUser.roleId == 3) {
      ...
      //  let tt=this.currentUser.
      // debugger

      this.workingdaysleft = this.currentUser.workkingdays;

      this.username = this.currentUser.org_name;
      for (let nav of this.currentUser.navigation) {


        if (nav.id == 7) {

          this.hospitalRoute = nav.url;
        }
    ...
    if (this.currentUser.roleId == 3) {
      ...
      if (this.currentUser.stage_id != 210) {
        let navmenu = this.currentUser.navigation;
        let indextoremove = navmenu.findIndex(x => x.id == 17)
        navmenu.splice(indextoremove, 1);

        if (this.currentUser.stage_id < 35) { // paymentdetails - menu
          let indextoremove = navmenu.findIndex(x => x.id == 16)
          navmenu.splice(indextoremove, 1);
        }
        ...
        this.menudy = this.currentUser.navigation;
      }

    }
    else {

      this.menudy = this.currentUser.navigation; // rrc
