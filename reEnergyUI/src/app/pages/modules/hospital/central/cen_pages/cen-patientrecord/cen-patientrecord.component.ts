...
import { CenPatientRecord } from 'src/app/pages/model/center/CenPatientRecord.model ';
...
@Component({
  selector: 'app-cen-patientrecord',
  templateUrl: './cen-patientrecord.component.html',
  styleUrls: ['./cen-patientrecord.component.scss', '../../central.component.scss']
})
export class CenPatientrecordComponent implements OnInit {
  openById = {}
  cenhospitalPages: cenHospitalPages;
  cenhospitalPagesQueBank: cenHospitalPagesQuestionBank;
  cenpatientrecord: CenPatientRecord;
  public currentObj: any;
  public propertyname: any;
  public selectedFile: any;
  cenpatientformSubmitted = false;
  ary_uploadedDocs: FileViewOrDelete[] = new Array();
  @Output() deleteFile = new EventEmitter();
  @ViewChild('acc') accordion: NgbAccordion;
  @ViewChild('cenpatientform', { static: false }) cenpatientform: NgForm;
  Stage_id: number;
  ...
  constructor(private cenhospdataserv: cenHospitalDataShareService, private hospdataserv: HospitalDataShareService, private modalService: NgbModal, private uploadService: FileUploadService, private tostr: CustomTosterServiceService) {
    ...
    this.cenpatientrecord = new CenPatientRecord();
    ...
    this.cenhospdataserv.getData().subscribe(data => {
      ...
      if (data != null || data != undefined) {
        ...
        this.cenpatientrecord = this.cenhospitalPages.cenpatientrecord;
      }
    ...
    this.cenhospdataserv.getQuestionBankData().subscribe(data => {
      ...
      if (data != null || data != undefined) {
        ...
        let tt = this.cenhospitalPagesQueBank.cenpatientrecord.guidelines_of_good_clinical_practice
        if (data.cenpatientrecord == undefined) {
          this.cenhospitalPagesQueBank.cenpatientrecord = new CenPatientRecord();
        }
    ...
    this.cenhospdataserv.getFormSubmissionStatus().subscribe(data => {
      ...
      if (data != null || data != undefined) {
        ...
        this.cenpatientformSubmitted = data;
  ...
  uncheckAllPtcare() {
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.name_of_doct = false;
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.patient_name = false;
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.uhid_number = false;
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.registration_no_doctor = false;
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.sign_of_doctor = false;
  }
  uncheckAllPtNone() {
    this.cenpatientrecord.patient_care_needs_and_regulatory_requirements.none = false;
  }
